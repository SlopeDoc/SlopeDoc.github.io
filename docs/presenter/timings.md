...
Slope can time your rehearsal so you can check the length of your talk. This is opt-in, through the ``--rehearse`` flag:
...
Press ``R`` at any time to reset all timings, and ``Space`` to pause/resume the timer, so a break is not counted in the section you stopped on.
